...
6. **Analytics — PostHog cloud ADOPTED (2026-07-22).** `web/src/instrumentation-client.ts` inits posthog-js against EU cloud (free 1M events/mo), auto pageviews/pageleaves, exception capture, DNT respected; no-op until NEXT_PUBLIC_POSTHOG_KEY is set (operator: create free project at eu.posthog.com, paste key into web/.env.local and Vercel env). YouTube channel + all 6 video descriptions carry UTM-tagged site links (utm_source=youtube, utm_medium=video/channel_about, utm_campaign=<building-slug>) so building-level attribution works from day one.

#!/usr/bin/env python3
"""Rollback canonical merge rows. Dry-run by default.

Two modes:
  * Fake/test mode (FAKE_ labels): unchanged Phase 3.8 behaviour.
  * Real mode (Phase 4, --real-ok): deletes the canonical contact(s) created by a
    single real merge batch and unlinks their methods/leads, behind strict guards.
    Real rollback additionally requires --apply --confirm-real-rollback and is
    REFUSED if the merge batch recorded communication_sent=true. Source audit rows
    (source_files, contact_import_rows) and review_action_log are never deleted;
    canonical_merge_links are preserved and only marked as rolled back. Counts only.
"""
...
SELECT
  ...
  count(*) FILTER (WHERE metadata->>'communication_sent' = 'true') AS comms_sent_count
FROM canonical_merge_batches
WHERE merge_label = {sql_literal(merge_label)};
"""


def real_dry_run_sql(merge_label: str) -> str:
    return f"""
WITH target AS (
  SELECT id
  FROM canonical_merge_batches
  WHERE merge_label = {sql_literal(merge_label)}
    AND is_test = false AND merge_label NOT LIKE 'FAKE_%'
),
target_contacts AS (
  SELECT c.id
  FROM contacts c
...
UNION ALL SELECT 'contact_methods_to_unlink', count(*) FROM contact_methods cm WHERE cm.contact_id IN (SELECT id FROM target_contacts)
UNION ALL SELECT 'lead_requirements_to_unlink', count(*) FROM lead_requirements lr WHERE lr.contact_id IN (SELECT id FROM target_contacts)
UNION ALL SELECT 'merge_links_to_mark', count(*) FROM canonical_merge_links cml WHERE cml.merge_batch_id IN (SELECT id FROM target)
ORDER BY item;
"""


def real_rollback_sql(merge_label: str) -> str:
    return f"""
BEGIN;
...
-- Refuse inside the transaction if comms were ever recorded as sent.
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM canonical_merge_batches
    WHERE id IN (SELECT id FROM tmp_target_merge)
      AND metadata->>'communication_sent' = 'true'
  ) THEN
    RAISE EXCEPTION 'Refusing rollback: communication_sent=true for this merge batch.';
  END IF;
END $$;
...
UPDATE contact_methods SET contact_id = NULL
WHERE contact_id IN (SELECT id FROM tmp_target_contacts);

UPDATE lead_requirements SET contact_id = NULL
WHERE contact_id IN (SELECT id FROM tmp_target_contacts);

-- Preserve links for audit; only mark them as rolled back (do not delete).
...
DELETE FROM contacts WHERE id IN (SELECT id FROM tmp_target_contacts);
...
UNION ALL SELECT 'remaining_real_contacts_for_merge', count(*) FROM contacts c
JOIN canonical_merge_batches cmb ON cmb.id = c.source_merge_batch_id
WHERE cmb.merge_label = {sql_literal(merge_label)} AND c.is_test = false
UNION ALL SELECT 'remaining_linked_contact_methods', count(*) FROM contact_methods cm
...
UNION ALL SELECT 'merge_links_preserved', count(*) FROM canonical_merge_links cml
JOIN canonical_merge_batches cmb ON cmb.id = cml.merge_batch_id
WHERE cmb.merge_label = {sql_literal(merge_label)}
ORDER BY item;
"""
...
def run_real_rollback(args, merge_batch_count: int, real_merge_count: int, comms_sent_count: int) -> int:
    if merge_batch_count != 1:
        print("Refusing rollback: expected exactly one canonical merge batch for label.")
        return 1
    if real_merge_count != 1:
        print("Refusing real rollback: expected exactly one real (non-test) merge batch for label.")
        return 1
    if args.merge_label.startswith("FAKE_"):
        print("Refusing real rollback: real merge label must not start with FAKE_.")
        return 1
    if comms_sent_count > 0:
        print("Refusing rollback: merge batch metadata has communication_sent=true.")
        return 1
    if not args.apply:
        print("Dry run only. No canonical rows were changed.")
        code, output = run_psql(real_dry_run_sql(args.merge_label))
        print(output)
        print("merge_batch_status_change|applied->rolled_back (would apply)")
        return code
    if not args.confirm_real_rollback:
        print("Refusing real rollback --apply: also pass --confirm-real-rollback.")
        return 1

    code, output = run_psql(real_rollback_sql(args.merge_label))
    print(output)
    return code


def main() -> int:
    parser = argparse.ArgumentParser(description="Rollback canonical merge rows. Dry-run by default.")
    ...
    if code != 0:
        print(status)
        return code
    fields = status.split("|") if status else ["0", "0", "0", "0"]
    merge_batch_count = int(fields[0] or 0)
    fake_merge_count = int(fields[1] or 0)
    real_merge_count = int(fields[2] or 0)
    comms_sent_count = int(fields[3] or 0) if len(fields) > 3 else 0

    if args.real_ok:
        return run_real_rollback(args, merge_batch_count, real_merge_count, comms_sent_count)

    if merge_batch_count != 1:
        print("Refusing rollback: expected exactly one canonical merge batch for label.")
        return 1
    if real_merge_count > 0 or fake_merge_count != 1 or not args.merge_label.startswith("FAKE_"):
        print("This is a real merge batch. Real rollback requires --real-ok.")
        return 1
